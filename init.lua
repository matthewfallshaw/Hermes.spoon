...
function obj.playpause()
  ...
  if state == obj.state_playing then
    obj.alertwithlog("Hermes playing", 3)
  elseif state == obj.state_paused or state == obj.state_stopped then
    obj.alertwithlog("Hermes paused", 3)
  else  -- unknown state
    return nil
  end
  return obj
...
function obj.next()
  if obj.isRunning() then
    tell('next song')
    obj.alertwithlog("Next song in Hermes", 3)
  else
    hs.alert.show("Hermes isn't running")
  end
  return obj
...
function obj.like()
  if obj.isRunning() then
    if obj.getCurrentRating() ~= 1 then
      tell('thumbs up')
      obj.alertwithlog("Liked song in Hermes", 3)
    else
      obj.alertwithlog("Song in Hermes already Liked", 3)
    end
    return obj
  else
    hs.alert.show("Hermes isn't running")
...
function obj.dislike()
  if obj.isRunning() then
    tell('thumbs down')
    obj.alertwithlog("Disliked song in Hermes", 3)
    return obj
  else
    hs.alert.show("Hermes isn't running")
  end
...
function obj.tired()
  if obj.isRunning() then
    tell('tired of song')
    obj.alertwithlog("Shelved song for a while in Hermes", 3)
    return obj
  else
    hs.alert.show("Hermes isn't running")
  end
...
function obj.quit()
  if obj.isRunning() then
    ...
    obj.alertwithlog("Quit Hermes", 3)
    return obj
  else
    hs.alert.show("Hermes isn't running")
  end
...
function obj.hide()
  if obj.isRunning() then
    ...
    if hermes:isHidden() then
      hermes:unhide()
      obj.alertwithlog("Unhid Hermes", 3)
    else
      hermes:hide()
      obj.alertwithlog("Hid Hermes", 3)
    end
    return obj
  else
    hs.alert.show("Hermes isn't running")
...
function obj:bindHotkeys(mapping)
  for k,v in pairs(mapping) do
    if self[k] and type(self[k]) == 'function' then
      ...
      self.hotkeys[k] = hs.hotkey.bind(mods, hotkey, function() self[k](self) end)
    else
